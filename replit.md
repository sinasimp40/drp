...
- **License System:**
    ...
    - **Trial Mode:** Public download links mint a one-shot 24h-capped trial license (`/api/trial_register`, signed). Launcher reports a stable per-PC fingerprint (SHA-256 of MachineGuid + motherboard + disk serial) and the server creates an auto-block (`trial_blocks` table) keyed on both IP and machine_hash. Two global settings on the **Trial Blocks** admin page (Management group): **Default cooldown** (default 7 days, 0 = disable auto-blocks) and **Download-link expiry** (default 24h, 0 = links live forever). Both are read live by `_auto_purge_inactive_trial_configs` / `/api/trial_register` so changes take effect immediately. Page also supports manual add/edit/delete with permanent and temporary blocks. Each license row stores its `machine_hash`, surfaced as a column on the History page.
