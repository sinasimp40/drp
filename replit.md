...
- **License System:**
    ...
    - **Trial Mode:** Public download links mint a one-shot 24h-capped trial license (`/api/trial_register`, signed). Launcher reports a stable per-PC fingerprint (SHA-256 of MachineGuid + motherboard + disk serial) and the server creates an auto-block (`trial_blocks` table) keyed on both IP and machine_hash. Each trial build has TWO independent settings on the Build config form: (1) **Trial duration** (`trial_duration_seconds`, default 24h, hard-capped at 24h) — the issued license lifetime after activation; (2) **Trial link expiry** (`trial_link_expiry_seconds`, NULL/blank/0 = never) — how long the launcher.exe + download page remain on the server. `_auto_purge_inactive_trial_configs` case (c) deletes a trial config (page + EXE + icon) only when `trial_link_expiry_seconds IS NOT NULL AND > 0 AND now - created_at > trial_link_expiry_seconds` — there is **no 24h fallback**; leaving the field blank means the link lives forever until manually deleted. The **Trial Blocks** admin page (Management group) holds only the **Default cooldown** (default 7 days, 0 = disable auto-blocks) and the manual block add/edit/delete UI. Each license row stores its `machine_hash`, surfaced as a column on the History page.
